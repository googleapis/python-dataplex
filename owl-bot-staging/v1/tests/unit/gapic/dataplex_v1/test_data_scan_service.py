...
def test_get_data_scan(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = datascans.DataScan(
            ...
            data_quality_spec=data_quality.DataQualitySpec(rules=[data_quality.DataQualityRule(range_expectation=data_quality.DataQualityRule.RangeExpectation(min_value='min_value_value'))]),
...
async def test_list_data_scans_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_data_scans(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_data_scan_jobs_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_data_scan_jobs(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
